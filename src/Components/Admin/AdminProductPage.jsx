...
const AdminProductPage = () => {
  ...
  const [edit, setEdit] = useState(false); //to control if the edit modal is shown
  const [Filter, setFilter] = useState('All');
  const [addProduct, setAddProduct] = useState(null)  // Tracks whether the "Add Product" modal is visible.
  ...
  // // Now addingData will receive the corrected value
  //   addingData(values)
  ...
  const handlesubmit = (values, { resetForm }) => {
    ...
    addingData(updatedValues);  // Pass the updated values to the function
    toast.success('Product added successfully!');
    resetForm();
  };

 //Opens the edit modal by setting the current product to edit state.
  ...
  const editHandleSubmit = (values) => {
    // Convert price and quantity to numbers.Converts edited data to correct types and submits to editFormData.
    const updatedValues = {
      ...values,
      price: Number(values.price),
      quantity: Number(values.quantity),
    ...
    editFormData(updatedValues);  
    setEdit(null);  // Close the edit form
  };
